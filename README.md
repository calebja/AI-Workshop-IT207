...
This "fancycat" command is a JavaScript implementation of the Linux "cat" command, with a couple changes. The original command displays the full contents of a file to the user.
This new command concatenates three files together, numbers all of the lines throughout all three files, and then displays the full, numbered combination of all three files to the user.
This command can be run using the syntax: node fancycat.js file1 file2 file3.
This command, per the assignment description, is "Extending a single command by adding an extra feature".
This command adds two extra features to the original cat command by allowing 3 files to be concatenated together, and by numbering all of the lines throughout all three concatenated files.
